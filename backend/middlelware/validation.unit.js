...
export const validateUnitCreate = (data) => {
    const schema = Joi.object({
        ...
        owner: Joi.string().pattern(/^[0-9a-fA-F]{24}$/).optional().allow(null).messages({
            "string.pattern.base": "Owner must be a valid user ID",
        }),
        primaryResident: Joi.string().pattern(/^[0-9a-fA-F]{24}$/).optional().allow(null).messages({
...
export const validateAssignResident = (data) => {
    const schema = Joi.object({
        ...
        role: Joi.string()
            ...
            .required()
            .messages({
                "any.only": "Role must be owner, member, or tenant",
